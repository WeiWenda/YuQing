...
public class Train {
    ...
    public static void main(String[] args) throws IOException, InvalidInputDataException {
        ...
        argv[4]="-e";
        argv[5]="0.001";
        argv[6]="./ProjectFile/train_input.txt";
        argv[7]="./Model.model";
        new Train().run(argv);
    }
    ...
    private void do_cross_validation() {
        ...
        if (param.solverType.isSupportVectorRegression()) {
            ...
            int[] denominator1=new int[5];    //准确率
            int[] denominator2=new int[5];    //召回率
            int l=0,m=0,n=0;
            for (int i = 0; i < prob.l; i++){
                //计算部分
            ...
            try
            {
                ...
                {
                    bw.write(String.valueOf(target[i]));
                    bw.newLine();
                }
                bw.flush();
                bw.close();
            ...
            //String[] type={"-1","1"};
            String[] type={"-1","1","2","3","4"};
            for (int i = 0; i < type.length; i++)
            {
                precision[i]=(double)numerator[i]/denominator1[i];
                recall[i]=(double)numerator[i]/denominator2[i];
                F_score[i]=(double)2*precision[i]*recall[i]/(precision[i]+recall[i]);
            }
            DecimalFormat df=new DecimalFormat("0.000");
            for (int i = 0; i < type.length; i++)
            {
                System.out.println(type[i]);
                System.out.println("准确率为："+df.format(precision[i]));
                System.out.println("召回率为："+df.format(recall[i]));
                System.out.println("F值为："+df.format(F_score[i]));
            }
            System.out.println("Positive");
            double p=(double)l/m;
            double r=(double)l/n;
            double f=(double)2*p*r/(p+r);
            System.out.println("准确率为："+df.format(p));
            System.out.println("召回率为："+df.format(r));
            System.out.println("F值为："+df.format(f));
     ...
     * @throws java.io.IOException obviously in case of any I/O exception ;)
